---
title: "PSC-012Y Course Syllabus"
format:
  html:
    toc: false
...
\textbf{Friday Lecture}\par
\medskip
\textbf{Meeting:} Friday Lecture\\
\textbf{Time(s):} 10:00AM -- 11:50AM\\
\textbf{Location:} Hart Hall 1116\\
\textbf{Prerequisites:} None

\medskip
\textbf{Weekly Virtual Tutorial}\par
\medskip
\textbf{Format:} Online / asynchronous\\
\textbf{Release:} Monday morning\\
\textbf{Purpose:} Guided coding practice in \texttt{R}
\end{minipage}\hfill
\begin{minipage}[t]{0.48\textwidth}
\textbf{Instructor}\par
\medskip
\textbf{Instructor:} Dr. William Rayo\\
\textbf{TA:} Suryadyuti Baral\\
\textbf{Course Email:} \texttt{datavis012Y@ucdavis.edu}\\
\textbf{Course Site:} \href{https://canvas.ucdavis.edu/courses/1089295}{Canvas}

\medskip
\textbf{Laboratory Section 1}\par
\medskip
\textbf{Lab Meeting:} Wednesday\\
\textbf{Time(s):} 4:40PM -- 6:00PM\\
\textbf{Location:} 73 Hutchison Hall

\medskip
\textbf{Laboratory Section 2}\par
\medskip
\textbf{Lab Meeting:} Wednesday\\
\textbf{Time(s):} 6:10PM -- 7:30PM\\
\textbf{Location:} 73 Hutchison Hall
\end{minipage}
:::
...
If your personal computer has difficulty running `R` and `RStudio`, you may use [Posit Cloud](https://posit.cloud/) (formerly RStudio Cloud) as a backup option. The first laboratory meeting is designed in part to help students get a local installation working or get a cloud-based environment running.
...
![Study design comparing `LLM`, search-engine, and brain-only groups across sessions.](images/ai-llm-study-design.png){fig-alt="Study design comparing LLM, search-engine, and brain-only groups across multiple sessions." width="85%"}

![Percentage of students in each group who struggled to quote anything from their essays, with the `LLM` group much higher than the search-engine or brain-only groups.](images/ai-llm-quote-results.png){fig-alt="Bar chart showing the percentage of students in each group who struggled to quote anything from their essays, with the LLM group much higher than the search-engine or brain-only groups." width="75%"}
...
\begin{longtable}{p{0.8in} p{1.2in} p{1.25in} p{1.1in} p{1.35in} p{1.7in} p{0.9in}}
\textbf{Phase} & \textbf{Friday Lecture} & \textbf{Virtual Tutorial Release} & \textbf{Wednesday Lab} & \textbf{Statistical Concept} & \textbf{Programming Concept} & \textbf{Optional Assignment(s)} \\
\hline
Setup &  &  & \textbf{April 1:} TA setup and installation support & Orientation to course tools and workflow & Local \texttt{R} / \texttt{RStudio} / \texttt{Quarto} installation or Posit Cloud setup &  \\
Unit 1 & \textbf{April 3:} Foundations in \texttt{R}, \texttt{Quarto}, and Tidy Data & \textbf{April 6:} Getting Started with Data in \texttt{R} & \textbf{April 8:} First Data Exploration Lab & Units of observation, variables, research questions, tidy data & \texttt{read\_csv()}, \texttt{glimpse()}, \texttt{select()}, \texttt{filter()}, first \texttt{ggplot()} &  \\
Unit 2 & \textbf{April 10:} Variable Types and Data Wrangling & \textbf{April 13:} Variable Types and Wrangling Practice & \textbf{April 15:} Wrangling Messy Survey Data & Categorical vs continuous, nominal vs ordinal, missing data & \texttt{mutate()}, \texttt{rename()}, \texttt{arrange()}, \texttt{count()}, \texttt{forcats} &  \\
Unit 3 & \textbf{April 17:} Categorical Data and Bar Charts & \textbf{April 20:} Categorical Data and Bar Charts & \textbf{April 22:} Categorical Analysis Lab & Counts, proportions, observed vs expected counts, introductory association testing & \texttt{count()}, grouped summaries, \texttt{geom\_bar()}, \texttt{chisq.test()} &  \\
Midterm 1 & \textbf{April 24:} Review and Synthesis for Midterm 1 & \textbf{April 27:} Midterm Review and Practice & \textbf{April 29:} \textbf{Midterm 1} & Review of Units 1 through 3 & Review of core \texttt{tidyverse} workflows and interpretation & Real-World Data Report I \\
Unit 4 & \textbf{May 1:} Continuous Data and Distributions & \textbf{May 4:} Continuous Data and Distributions & \textbf{May 6:} Continuous Data Lab & Center, spread, skew, outliers & \texttt{summarise()}, \texttt{mean()}, \texttt{median()}, \texttt{sd()}, \texttt{IQR()}, histograms, density plots, boxplots &  \\
Unit 5 & \textbf{May 8:} Uncertainty and Comparing Groups & \textbf{May 11:} Uncertainty and Comparing Groups in \texttt{R} & \textbf{May 13:} Group Comparison Lab & Sampling variability, confidence intervals, two-group and multi-group comparisons, p-values in plain language & Summary tables, mean plots, error bars, \texttt{t.test()}, \texttt{aov()} &  \\
Unit 6 & \textbf{May 15:} Relationships Between Continuous Variables & \textbf{May 18:} Relationships Between Continuous Variables & \textbf{May 20:} Scatterplots and Trend Lab & Association, trend, correlation, regression as line-of-best-fit intuition & Scatterplots, \texttt{geom\_smooth(method = "lm")}, \texttt{cor()}, simple \texttt{lm()} output reading &  \\
Unit 7 & \textbf{May 22:} Free-Response Text as Data & \textbf{May 26:} Free-Response Text as Data (released Tuesday because of Memorial Day) & \textbf{May 27:} Introductory Text Analysis Lab & Word frequency, stop words, sentiment, strengths and limits of word clouds & \texttt{unnest\_tokens()}, stop-word removal, \texttt{count()}, joins, word clouds, simple sentiment summaries &  \\
Midterm 2 & \textbf{May 29:} Review, Communication, and Midterm 2 Preparation & \textbf{June 1:} Final Review and Results Writing Practice & \textbf{June 3:} \textbf{Midterm 2} & Review of Units 4 through 7; choosing the right figure and analysis & Synthesis of continuous distributions, uncertainty, group comparisons, continuous relationships, and text-analysis workflows & Real-World Data Report II \\
Wrap-Up & \textbf{June 5:} Course Synthesis and Wrap-Up &  &  & Communicating with data, reflecting on what we learned, connecting course skills to future work & Pulling together a complete workflow from question to interpretation &  \\
\end{longtable}
\normalsize
\end{landscape}
:::
